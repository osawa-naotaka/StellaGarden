...
## 9. 11_TOOL.md への影響（更新が必要な箇所）

この仕様を採用するにあたり、`doc/11_TOOL.md` の以下の記述を後続タスクで更新する：

- §1.3 中間素材テーブル: 刃のレシピを `隕鉄 →【炉】→ 赤熱した隕鉄 →【金床】→ 刃` に書き換え
- §3.3 刃の鍛造レシピ（L163-167）: トング・ハンマーを経由しない新フローに書き換え
- §3.4 鍛造の手順（L179-191）: 新しい2ステップフローに書き換え
- §6.2 / §7.2 推奨建設順序: トング・石のハンマーを必須から外す（レシピは残す）
- §9 今後の検討事項: ✅ UI/UX 設計完了として本書へのリンクを追加

本ドキュメント（20_FORGE.md）が正となり、差分は後続タスクで反映する。
