...
registerEntity({
    ...
    onDailyTick(ctx: DailyTickContext): void {
        applyCropDailyTick(ctx, CROP_DEFS[ENTITY_TYPES.sunflower]);
    },
    ...
    onInteract(ctx: InteractionContext): boolean {
        ...
        if (
            !ctx.inventory.addItems([
                { itemId: "sunflower_seed", count: harvestCount },
...
registerItem({
    ...
    onItemUse(ctx: InteractionContext): boolean {
        ...
        let newVoxel = BigInt(terrainType) | (BigInt(ENTITY_TYPES.sunflower) << 8n);
