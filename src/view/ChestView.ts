...
/** スロットの場所を識別する型。area = "inventory" | "toolbar" | "chest" */
type ChestSlotArea = "inventory" | "toolbar" | "chest";
type ChestSlotRef = { area: ChestSlotArea; index: number };

/**
 * チェストUI: 左にプレイヤーインベントリ、右にチェストの中身を表示する。
 * UIState.mode === "chest" のとき表示し、Escape で閉じる。
 */
export class ChestView {
    ...
    private chestSlotIcons: SlotIcon[] = [];

    private pickedUp: { stack: ItemStack; source: ChestSlotRef } | null = null;
    ...
    constructor(inventory: IInventoryWriter, chestStorage: ChestStorage, uiState: UIState) {
        ...
        const chestTitle = new BitmapText({ text: "Chest", style: { fontFamily: "Roboto", fontSize: 14, fill: 0xffffff } });
        chestTitle.x = rightX;
        chestTitle.y = PADDING;
        this.container.addChild(chestTitle);

        const chestY = PADDING + TITLE_HEIGHT;
        for (let row = 0; row < ROWS; row++) {
            for (let col = 0; col < COLS; col++) {
                const idx = row * COLS + col;
                const slotContainer = this.createSlotContainer(rightX + col * CELL_SIZE, chestY + row * CELL_SIZE, { area: "chest", index: idx });
                this.container.addChild(slotContainer);
                this.chestSlotIcons.push(createSlotIcon(slotContainer));
            }
        }
    ...
    tick(): void {
        ...
        const chestPos = this.uiState.chestPos;
        if (!chestPos) return;
        ...
        // チェストスロット更新
        for (let i = 0; i < ROWS * COLS; i++) {
            updateSlotIcon(this.chestSlotIcons[i], this.chestStorage.getSlot(chestPos, i));
        }
    ...
    private createSlotContainer(x: number, y: number, ref: ChestSlotRef): Container {
    ...
    private getSlotByRef(ref: ChestSlotRef): ItemStack | null {
        if (ref.area === "chest") {
            const pos = this.uiState.chestPos;
            if (!pos) return null;
            return this.chestStorage.getSlot(pos, ref.index);
        }
        return this.inventory.getSlot(ref as SlotRef);
    }

    private setSlotByRef(ref: ChestSlotRef, stack: ItemStack | null): void {
        if (ref.area === "chest") {
            const pos = this.uiState.chestPos;
            if (!pos) return;
            this.chestStorage.setSlot(pos, ref.index, stack);
            return;
        }
        this.inventory.setSlot(ref as SlotRef, stack);
    }

    private handleLeftClick(ref: ChestSlotRef): void {
    ...
    private handleRightClick(ref: ChestSlotRef): void {
        if (!this.pickedUp) {
            // 1個だけピックアップ
